...
try 
    ...
    data = cell(N_haproxy,20);
    sessionsList = cell(1,N_haproxy);
    sessionsListInd = cell(1,N_haproxy);
...
%         str = line;
        ...
        if ~isempty(strfind(output{1,1}{1,14},'JSESSIONID'))
            ...
            if isempty(sessionsListInd{1,frontendID})
                sessionsListInd{1,frontendID} = ArrayList;
            end
            frontendID = frontendList.indexOf(frontend) + 1;
            ...
            pos = str.indexOf('/',0);
            pos_start = str.indexOf('/',pos+1);
            pos_end = str.indexOf('/',pos_start+1);
            delay = str2double(str.substring(pos_start+1,pos_end))/1000;
            
            pos_end2 = str.indexOf('/',pos_end+1);
            res = str2double(str.substring(pos_end+1,pos_end2))/1000;
            response = delay + res;
            
            if size(data{1,serverIDAll},2) < categoryIndex
                data{1,serverIDAll}{7,categoryIndex} = [];
            end

            data{1,serverIDAll}{3,categoryIndex} = [data{1,serverIDAll}{3,categoryIndex};arrival];
            data{1,serverIDAll}{4,categoryIndex} = [data{1,serverIDAll}{4,categoryIndex};response-delay];
            data{1,serverIDAll}{7,categoryIndex} = [data{1,serverIDAll}{7,categoryIndex};delay];
            ...
            if sessionIDList{1,frontendID}.contains(sessionID)
                ...
                sessionsListInd{1,frontendID}.get(index).add(serverID);
                ...
                temp = ArrayList;
                temp.add(serverID);
                sessionsListInd{1,frontendID}.add(temp);
                index = sessionIDList{1,frontendID}.indexOf(sessionID);
...
max_length = 0;
fprintf(fileID,'two point five check: %s\n',toc);
...
for i = 1:size(data,2)
    if ~isempty(data{1,i})
        ...
        index = str2double(regexprep(server_id,'[a-zA-Z]',''))
...
for s = 1:frontendList.size
    data_session_all = data_session;
    data_session_all{1,s}{2,size(data_session_all{1,s},2)+1} = zeros(10,1);
    data_session_all{1,s} = dataFormat(data_session_all{1,s},window);
end
...
save(strcat(filePath,'workspace.mat'),'sessionStart','requestTimes','D_request','serverIDListAll','sessionsList','data','sessionIDList','frontendList','nCPU','warmUp','N_haproxy','data_session','uniSessions')
...
save(strcat(filePath,'LBData.mat'),'uniSessions','sessionsList','sessionIDList','sessionTimes','thinkTimes','requestTimes','N','D','Z','frontendList','serverIDList','sessionStart','categoryList','N','R','X','uniArray','Z_request','serverIDListAll','data_session','data','D_request','old_value','frontendResourceMap','data_session_all','sessionsListInd')
save(strcat(filePath,'LBData',num2str(count_file),'.mat'),'uniSessions','sessionsList','sessionIDList','sessionTimes','thinkTimes','requestTimes','N','D','Z','frontendList','serverIDList','sessionStart','categoryList','N','R','X','uniArray','Z_request','serverIDListAll','data_session','data','D_request','old_value','frontendResourceMap','data_session_all','sessionsListInd')
