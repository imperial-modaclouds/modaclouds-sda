...
for s = 1:frontendList.size
    for i = 1:serverIDList{1,s}.size
        flag = 0;
        for j = 1:size(cpuUtilTarget,2)
            if ~isempty(strfind(char(cpuUtilTarget{1,j}),char(serverIDList{1,s}.get(i-1))))
                flag = 1;
                cpu = obj.obtainData(java.lang.String(cpuUtilTarget{1,j}),java.lang.String('TrickCPUUtilization'));
                if isempty(cpu)
                    break;
                end
                cpu_value{1,i} = convertArrayList(cpu.getValues);
                cpu_timestamps{1,i} = convertArrayList(cpu.getTimestamps);
                break;
            end
        end
        if flag == 0
            disp(strcat('CPU utilization is not collected from server: ',char(serverIDList{1,s}.get(i-1))))
            validate = 0;
        end
    end
end
...
save('/home/ubuntu/sda/workspace.mat','sessionStart','requestTimes','sessionsList','data','sessionIDList','frontendID','frontendList','nCPU','method','warmUp','cpu_value','cpu_timestamps','serverIDList')
...
if frontendList.size > 1
    for i = 1:serverIDList{1,1}.size
        for j = 1:serverIDList{1,2}.size
            if strcmp(serverIDList{1,1}.get(i-1),serverIDList{1,2}.get(j-1))
                data_combined{1,i} = integrate(data{1,i},data{2,j});
                break;
            end
        end
        data_combined{1,i}{2,size(data_combined{1,i},2)+1} = [];
        [data_combined{1,i},~] = dataFormat(data_combined{1,i}, window, [], cpu_value{1,i}, cpu_timestamps{1,i});
    end
end
    ...
    for s = 1:frontendList.size
        if ~isempty(data{s,i})
            ...
            switch method
                ...
                case 'fcfs'
                    D_request{s,i} = fcfs(data_combined{1,i},nCPU(i),avgWin);
                case 'ubo'
                    D_request{s,i} = ubo(data_combined{1,i},maxTime);
                case 'ubr'
                    D_request{s,i} = ubr(data_combined{1,i},nCPU(i));
                case 'otherwise'
...
function combined = integrate(data1, data2)
combined = cell(6,size(data1,2));

for si = 1:size(data1,2)
    combined{3,si} = [combined{3,si};data1{3,si};data2{3,si}];
    combined{4,si} = [combined{4,si};data1{4,si};data2{4,si}];
end
end
